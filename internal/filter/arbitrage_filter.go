...
func (f *arbitrageFilter) Execute() {
	...
	for res := range results {
		if len(res.Selections) == 2 {
			...
			f.db.Exec(q1, res.Group.Id, res.Group.Market, res.Group.GroupingKey, keys[0], keys[1], x, y, totalImpliedProb, vig)
			...
			vig := totalImpliedProb - 1
			f.db.Exec(q2, res.Group.Id, res.Group.Market, res.Group.GroupingKey, keys[0], keys[1], keys[2], x, y, z, totalImpliedProb, vig)
		}
	}
}
